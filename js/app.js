...
function activeActors(){const source=window.RDA_ACTORS||window.ACTORS||[];if(window.RDA?.configured)return source;const hidden=JSON.parse(localStorage.getItem('rda_hidden')||'[]');return source.filter(a=>!hidden.includes(a.id))}
function actorAge(a){return Number.isFinite(Number(a.age))?Number(a.age):ageFromBirth(a.birth)}
...
function studentIcon(){return `<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M2.7 9.2 12 4l9.3 5.2L12 14.4 2.7 9.2Z"/><path d="M6.2 11.3v4.2c0 1.5 2.6 3.1 5.8 3.1s5.8-1.6 5.8-3.1v-4.2"/><path d="M21.3 9.2v5.2"/></svg>`}
function checkIcon(){return `<svg viewBox="0 0 24 24" aria-hidden="true"><path d="m5 12.5 4.2 4.2L19 7"/></svg>`}
function mailIcon(){return `<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M3.5 6.5h17v11h-17z"/><path d="m4 7 8 6 8-6"/></svg>`}
...
function openContactForm(actor){
  ...
  if(!modal){
    ...
    modal.innerHTML=`<div class="contact-modal-card"><h2>Contactar a ${actor.name}</h2><p>Tu mensaje se envía de forma privada. Los datos personales del actor no se muestran públicamente.</p><form id="contactForm"><div class="field"><label>Tu nombre</label><input id="contactName" required maxlength="100"></div><div class="field"><label>Tu email</label><input id="contactEmail" type="email" required maxlength="200"></div><div class="field"><label>Mensaje</label><textarea id="contactMessage" rows="5" required minlength="10" maxlength="1200" placeholder="Contale brevemente sobre el proyecto y cómo contactarte."></textarea></div><div class="contact-modal-actions"><button class="btn btn-secondary" type="button" data-close-contact>Cancelar</button><button class="btn btn-primary" type="submit">Enviar contacto</button></div></form></div>`;
...
document.addEventListener('DOMContentLoaded',async()=>{initNav();if(window.RDA?.configured){try{window.RDA_ACTORS=await RDA.publicActors()}catch(e){console.error(e);window.RDA_ACTORS=[]}}renderHome();await renderActor();initModal();if(!window.RDA?.configured)initAdmin()});
