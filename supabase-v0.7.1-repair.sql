-- RED DE ACTORES — V0.7.1 REPAIR
-- Ejecutar UNA VEZ si V0.7 muestra "Bucket not found" o
-- "column castings.submitted_at does not exist".
-- Es idempotente: puede ejecutarse aunque parte de V0.7 ya exista.

-- 1) Completar columnas necesarias en castings.
...
-- Si description existía como obligatorio en una versión anterior, permitir null.
...
drop policy if exists "public reads approved active castings" on public.castings;
...
-- 3) Bucket para imágenes de castings.
...
-- 4) RPC pública para enviar casting sin cuenta.
